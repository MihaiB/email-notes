...
if exists "Delivered-To" {
	# incoming
	if header :comparator "i;octet" :is
	"Delivered-To" "RoadRunner@example.org" {
		fileinto "RR-label";
	} elsif header :comparator "i;octet" :is
	"Delivered-To" "bird@example.org" {
		fileinto "bird-label";
	} else {
	...
	"From" ["RoadRunner@example.org", "RoadRunner+*@example.org"] {
		fileinto "RR-label";
	} elsif address :comparator "i;octet" :all :matches
